...
def find_patient_by_phone_name_and_species(patients, owner_phone, patient_name, species):
    for patient in patients:
        if (
            patient["owner_phone"].strip() == owner_phone.strip()
            and patient["patient_name"].strip().lower() == patient_name.strip().lower()
            and patient["species"].strip().lower() == species.strip().lower()
        ):
            return patient
    return None
...
def list_patients(patients):
    ...
    for patient in patients:
        print(
            ...
            f"Mascota: {patient['patient_name']} ({patient['species']}) | "
            f"Tutor: {patient['owner_name']} | "
            f"Tel: {patient['owner_phone']} | "
            f"Edad: {patient['age']}"
        )
